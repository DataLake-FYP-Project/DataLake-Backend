...
ES_INDEX = "datalake-animal-data"


def animal_upload_to_minio(file_path):
    s3 = boto3.client(
        's3',
    ...
    try:
        folder_path = f"animal_detection/"
...
def animal_upload_to_elasticsearch(file_path):
    try:
        ...
        for i, detection in enumerate(detections):
            ...
            es.index(index=ES_INDEX, id=i + 1, body=record, pipeline="animal_data_timestamp_pipeline")
            print(f"Uploaded doc {i + 1} to Elasticsearch")

    except Exception as e:
        print(f"Error uploading animal data: {e}")
