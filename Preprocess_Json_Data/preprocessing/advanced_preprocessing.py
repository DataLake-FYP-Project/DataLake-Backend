    ...
    def process_all(self, input_bucket: str, output_bucket: str):
        ...
        try:
            ...
            for file in people_files:
                processed_df = self._process_file(self.people_processor, input_bucket, output_bucket, file, "people_detection")
                if processed_df:
                    grouped = self.people_processor._group_data(processed_df)
                    enriched_data = dict(sorted([self.people_processor._enrich_person(row) for row in grouped.collect()], 
        ...
        try:
            ...
            for file in vehicle_files:
                processed_df = self._process_file(self.vehicle_processor, input_bucket, output_bucket, file, "vehicle_detection")
                if processed_df:
                    grouped = self.vehicle_processor._group_data(processed_df)
                    enriched_data = dict(sorted([self.vehicle_processor._enrich_vehicle(row) for row in grouped.collect()], 
                                             key=lambda x: int(x[0])))
                    output = self._get_common_output_structure(file)
                    output.update({
